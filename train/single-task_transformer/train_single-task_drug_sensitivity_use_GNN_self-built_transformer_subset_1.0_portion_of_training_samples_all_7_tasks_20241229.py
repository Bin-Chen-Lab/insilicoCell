...
class Model(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None, CLS_token = None):
        ...
        embeddings_4 = torch.cat((CLS_token.repeat(sample_size_4, 1), all_drug[:sample_size_4, :], all_cellline[:sample_size_4, :]), dim=1)
        #No padding for this task.
        embeddings_4 = embeddings_4.reshape(sample_size_4, 3, self.hidden_size) #torch.Size([128, 3, 768])
        # Add attention mask:
        attention_mask_4 = (torch.tensor([0, 0, 0], dtype=torch.float32)).repeat(sample_size_4, 1).to(device)
        ...
        pooled_output_4 = pooled_output[:sample_size_4, :]
...
#X_training_drug_sensitivity = torch.load('/egr/research-aidd/chenruo4/self-built_transformer/input_representations/concat_input_per_task/X_training_set_task_drug_sensitivity_use_GNN_use_original_AUC.pt')
...
#Y_training_drug_sensitivity = torch.load('/egr/research-aidd/chenruo4/self-built_transformer/input_representations/concat_input_per_task/Y_training_set_task_drug_sensitivity_use_GNN_use_original_AUC.pt')
...
for e in range(epoch):
    ...
    for batch_4 in train_loader_4:
        ...
        optimizer.zero_grad()
        #
        outputs_4 = model(inputs_embeds = [input_data_4.to(device)], 
                          input_task_id = [task_id_4], 
                          CLS_token = CLS.to(device))
        #
        loss_4 = loss_regression(outputs_4.view(-1), labels_4.to(device).view(-1))
        #weight the loss to around 1.5:
        #loss = (loss_1 * 0.5) + (loss_2 * 0.05) + (loss_3 * 2) + (loss_4 * 0.01) + (loss_5 * 3) + (loss_6 * 2) + (loss_7)
        loss = loss_4
    ...
    # Save the model:
    #torch.save(model.state_dict(), out_dir + 'self-built_transformer_no_modality_tokens_single-task_drug_sensitivity_use_GNN_use_original_AUC_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    torch.save(model.state_dict(), out_dir + 'self-built_transformer_no_modality_tokens_single-task_drug_sensitivity_use_GNN_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    # Save training loss:
    df = pd.DataFrame({'epoch_total_loss': epoch_loss_all}, index = range(0, len(epoch_loss_all)))
    #df.to_csv(out_dir + 'training_loss_self-built_transformer_no_modality_tokens_single-task_drug_sensitivity_use_GNN_use_original_AUC_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
    df.to_csv(out_dir + 'training_loss_self-built_transformer_no_modality_tokens_single-task_drug_sensitivity_use_GNN_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
