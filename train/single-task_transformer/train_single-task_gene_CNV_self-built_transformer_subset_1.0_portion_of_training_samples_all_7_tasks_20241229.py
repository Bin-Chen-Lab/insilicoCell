...
for e in range(epoch):
    ...
    # Save the model:
    torch.save(model.state_dict(), out_dir + 'self-built_transformer_no_modality_tokens_single-task_gene_CNV_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    # Save training loss:
    df = pd.DataFrame({'epoch_total_loss': epoch_loss_all}, index = range(0, len(epoch_loss_all)))
    df.to_csv(out_dir + 'training_loss_self-built_transformer_no_modality_tokens_single-task_gene_CNV_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
