...
class Model(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None, CLS_token = None):
        ...
        embeddings_3 = torch.cat((CLS_token.repeat(sample_size_3, 1), all_gene[:sample_size_3, :], all_protein[:sample_size_3, :]), dim=1)
        embeddings_3 = embeddings_3.reshape(sample_size_3, 3, self.hidden_size) #torch.Size([128, 3, 768])
        # Add attention mask:
        attention_mask_3 = (torch.tensor([0, 0, 0], dtype=torch.float32)).repeat(sample_size_3, 1).to(device)
        #
        ...
        pooled_output_3 = pooled_output[:sample_size_3, :]
...
train_loader_3 = DataLoader(train_dataset_TF_gene_regulation, batch_size = num_sample_per_batch, shuffle = True)
...
for e in range(finetune_epoch):
    ...
    # Save the model:
    torch.save(model.state_dict(), out_dir + 'finetune_self-built_transformer_no_modality_tokens_single-task_TF_gene_regulation_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    # Save training loss:
    df = pd.DataFrame({'epoch_total_loss': epoch_loss_all}, index = range(0, len(epoch_loss_all)))
    df.to_csv(out_dir + 'finetune_loss_self-built_transformer_no_modality_tokens_single-task_TF_gene_regulation_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
