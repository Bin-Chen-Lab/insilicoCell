...
class Model(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None, CLS_token = None):
        ...
        embeddings_5 = torch.cat((CLS_token.repeat(sample_size_5, 1), all_cellline[:sample_size_5, :], all_gene[:sample_size_5, :]), dim=1)
        embeddings_5 = embeddings_5.reshape(sample_size_5, 3, self.hidden_size) #torch.Size([128, 6, 768])
        # Add attention mask:
        attention_mask_5 = (torch.tensor([0, 0, 0], dtype=torch.float32)).repeat(sample_size_5, 1).to(device)
        #
        ...
        pooled_output_5 = pooled_output[:sample_size_5, :]
...
train_loader_5 = DataLoader(train_dataset_gene_effect_score, batch_size = num_sample_per_batch, shuffle = True)
...
for e in range(finetune_epoch):
    ...
    # Save the model:
    torch.save(model.state_dict(), out_dir + 'finetune_self-built_transformer_no_modality_tokens_single-task_gene_effect_score_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    # Save training loss:
    df = pd.DataFrame({'epoch_total_loss': epoch_loss_all}, index = range(0, len(epoch_loss_all)))
    df.to_csv(out_dir + 'finetune_loss_self-built_transformer_no_modality_tokens_single-task_gene_effect_score_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
