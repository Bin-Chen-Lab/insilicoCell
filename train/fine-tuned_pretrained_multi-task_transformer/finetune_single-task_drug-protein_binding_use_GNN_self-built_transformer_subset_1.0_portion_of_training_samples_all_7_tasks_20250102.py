...
class Model(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None, CLS_token = None):
        ...
        embeddings_2 = torch.cat((CLS_token.repeat(sample_size_2, 1), all_drug[:sample_size_2, :], all_protein[:sample_size_2, :]), dim=1)
        #No padding for this task.
        embeddings_2 = embeddings_2.reshape(sample_size_2, 3, self.hidden_size) #torch.Size([128, 3, 768])
        # Add attention mask:
        attention_mask_2 = (torch.tensor([0, 0, 0], dtype=torch.float32)).repeat(sample_size_2, 1).to(device)
        #
        ...
        pooled_output_2 = pooled_output[:sample_size_2, :]
...
train_loader_2 = DataLoader(train_dataset_drug_protein_binding, batch_size = num_sample_per_batch, shuffle = True)
...
for e in range(finetune_epoch):
    ...
    # Save the model:
    torch.save(model.state_dict(), out_dir + 'finetune_self-built_transformer_no_modality_tokens_single-task_drug-protein_binding_use_GNN_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229')
    # Save training loss:
    df = pd.DataFrame({'epoch_total_loss': epoch_loss_all}, index = range(0, len(epoch_loss_all)))
    df.to_csv(out_dir + 'finetune_loss_self-built_transformer_no_modality_tokens_single-task_drug-protein_binding_use_GNN_lr0.00001_all_7_tasks_subset_1.0_portion_' + str(e) + '_20241229.csv')
