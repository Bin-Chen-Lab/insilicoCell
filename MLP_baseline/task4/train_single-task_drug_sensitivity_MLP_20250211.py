...
class MLPModel(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None):
        ...
        embeddings_4 = torch.cat((all_drug[:sample_size_4, :], all_cellline[:sample_size_4, :]), dim=1)
        #
        ...
        out_4 = self.regressor_4(embeddings)
        return out_4
...
X_training_drug_sensitivity = torch.load('../../../X_training_set_task_drug_sensitivity_use_GNN.pt')
#torch.Size([276712, 728])
Y_training_drug_sensitivity = torch.load('../../../Y_training_set_task_drug_sensitivity_use_GNN.pt')

X_val_drug_sensitivity = torch.load('../../../X_test_set_seen_unseen_mixture_task_drug_sensitivity_use_GNN.pt')
Y_val_drug_sensitivity = torch.load('../../../Y_test_set_seen_unseen_mixture_task_drug_sensitivity_use_GNN.pt')

X_test_drug_sensitivity = torch.load('../../../X_test_set_unseen_task_drug_sensitivity_use_GNN.pt')
Y_test_drug_sensitivity = torch.load('../../../Y_test_set_unseen_task_drug_sensitivity_use_GNN.pt')
...
epoch = 100
rdseed = 10 
out_dir = '/egr/research-aidd/molinqin/insilicoCell_Ruoqiao/concat_input_per_task/mlp/optimized_singletask_MLP/task4/'

#---------------------------------------------------------------------------------------
num_sample_per_batch_4 = 1024

train_loader_4 = DataLoader(train_dataset_drug_sensitivity, batch_size = num_sample_per_batch_4, shuffle = True)
val_loader_4 = DataLoader(val_dataset_drug_sensitivity, batch_size = num_sample_per_batch_4, shuffle = False)
test_loader_4 = DataLoader(test_dataset_drug_sensitivity, batch_size = num_sample_per_batch_4, shuffle = False)
...
for e in range(epoch):
    ...
    total_batches = len(train_loader_4)
    #
    for batch_4 in tqdm(train_loader_4, desc=f"Epoch {e+1}/{epoch} - Training", total=total_batches):
        #
        #task 4: 'drug_sensitivity':
    ...
    #
    df_val = pd.DataFrame({"y_pred": val_outputs, "y_truth": Y_val_drug_sensitivity.detach().numpy().flatten()})
    ...
    avg_test_loss = running_test_loss / len(test_loader_4)
    test_loss_list.append(avg_test_loss)
    #
    df_test = pd.DataFrame({"y_pred": test_outputs, "y_truth": Y_test_drug_sensitivity.detach().numpy().flatten()})
...
torch.save(best_model_wts, out_dir + f'MLP_drug_sensitivity_best_model_epoch_{best_epoch}_20250211.pth')
