...
class MLPModel(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None):
        ...
        embeddings_3 = torch.cat((all_gene[:sample_size_3, :], all_protein[:sample_size_3, :]), dim=1)
        #
        ...
        out_3 = F.sigmoid(self.regressor_3(embeddings))
        return out_3
...
X_training_TF_gene_regulation = torch.load('../../../X_training_set_task_TF-gene_regulation_balanced_data.pt')
#torch.Size([2774198, 1152])
Y_training_TF_gene_regulation = torch.load('../../../Y_training_set_task_TF-gene_regulation_balanced_data.pt')

X_val_TF_gene_regulation = torch.load('../../../X_test_set_seen_unseen_mixture_task_TF-gene_regulation.pt')
Y_val_TF_gene_regulation = torch.load('../../../Y_test_set_seen_unseen_mixture_task_TF-gene_regulation.pt')

X_test_TF_gene_regulation = torch.load('../../../X_test_set_unseen_task_TF-gene_regulation.pt')
Y_test_TF_gene_regulation = torch.load('../../../Y_test_set_unseen_task_TF-gene_regulation.pt')
...
epoch = 100
rdseed = 10 
out_dir = '/egr/research-aidd/molinqin/insilicoCell_Ruoqiao/concat_input_per_task/mlp/optimized_singletask_MLP/task3_balanced/'

#---------------------------------------------------------------------------------------
num_sample_per_batch_3 = 1024

train_loader_3 = DataLoader(train_dataset_TF_gene_regulation, batch_size = num_sample_per_batch_3, shuffle = True)
val_loader_3 = DataLoader(val_dataset_TF_gene_regulation, batch_size = num_sample_per_batch_3, shuffle = False)
test_loader_3 = DataLoader(test_dataset_TF_gene_regulation, batch_size = num_sample_per_batch_3, shuffle = False)
...
for e in range(epoch):
    ...
    total_batches = len(train_loader_3)
    #
    for batch_3 in tqdm(train_loader_3, desc=f"Epoch {e+1}/{epoch} - Training", total=total_batches):
        #
        #task 3: 'TF-gene_regulation':
        ...
        optimizer.zero_grad()
        #
        outputs_3 = model(inputs_embeds = [input_data_3.to(device)], 
                          input_task_id = [task_id_3])
        #
    ...
    avg_test_loss = running_test_loss / len(test_loader_3)
...
df_all_performance.to_csv(out_dir + 'MLP_TF_gene_regulation_performance_all_epochs_20250211.csv', index=False)
...
torch.save(best_model_wts, out_dir + f'MLP_TF_gene_regulation_best_model_epoch_{best_epoch}_20250211.pth')
