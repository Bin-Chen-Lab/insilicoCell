...
class MLPModel(nn.Module):
    ...
    def forward(self, inputs_embeds, input_task_id = None):
        ...
        embeddings_5 = torch.cat((all_cellline[:sample_size_5, :], all_gene[:sample_size_5, :]), dim=1)
        #
        ...
        out_5 = self.regressor_5(embeddings)
        return out_5
...
X_training_gene_effect_score = torch.load('../../../X_training_set_task_gene_effect_score.pt')
#torch.Size([8809512, 256])
Y_training_gene_effect_score = torch.load('../../../Y_training_set_task_gene_effect_score.pt')

X_val_gene_effect_score = torch.load('../../../X_test_set_seen_unseen_mixture_task_gene_effect_score.pt')
Y_val_gene_effect_score = torch.load('../../../Y_test_set_seen_unseen_mixture_task_gene_effect_score.pt')

X_test_gene_effect_score = torch.load('../../../X_test_set_unseen_task_gene_effect_score.pt')
Y_test_gene_effect_score = torch.load('../../../Y_test_set_unseen_task_gene_effect_score.pt')
...
epoch = 100
rdseed = 10 
out_dir = '/egr/research-aidd/molinqin/insilicoCell_Ruoqiao/concat_input_per_task/mlp/optimized_singletask_MLP/task5/'

#---------------------------------------------------------------------------------------
num_sample_per_batch_5 = 2048

train_loader_5 = DataLoader(train_dataset_gene_effect_score, batch_size = num_sample_per_batch_5, shuffle = True)
val_loader_5 = DataLoader(val_dataset_gene_effect_score, batch_size = num_sample_per_batch_5, shuffle = False)
test_loader_5 = DataLoader(test_dataset_gene_effect_score, batch_size = num_sample_per_batch_5, shuffle = False)
...
for e in range(epoch):
    ...
    total_batches = len(train_loader_5)
    #
    for batch_5 in tqdm(train_loader_5, desc=f"Epoch {e+1}/{epoch} - Training", total=total_batches):
        #
        #task 5: 'gene_effect_score'
        ...
        optimizer.zero_grad()
        #
        outputs_5 = model(inputs_embeds = [input_data_5.to(device)], 
                          input_task_id = [task_id_5])
        #
    ...
    avg_val_loss = running_val_loss / len(val_loader_5)
    val_loss_list.append(avg_val_loss)
    #
    df_val = pd.DataFrame({"y_pred": val_outputs, "y_truth": Y_val_gene_effect_score.detach().numpy().flatten()})
    ...
    avg_test_loss = running_test_loss / len(test_loader_5)
    test_loss_list.append(avg_test_loss)
    #
    df_test = pd.DataFrame({"y_pred": test_outputs, "y_truth": Y_test_gene_effect_score.detach().numpy().flatten()})
...
# Save the model:
torch.save(best_model_wts, out_dir + f'MLP_gene_effect_score_best_model_epoch_{best_epoch}_20250211.pth')
